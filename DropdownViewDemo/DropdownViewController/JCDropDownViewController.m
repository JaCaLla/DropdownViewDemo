...
- (void)setupUIControls:(UIView*)aUpperView button:(UIButton*)aButton{
    ...
    self.svwUpperSubview = [[UIView alloc] initWithFrame:aUpperSubviewFrame];
    self.svwUpperSubview.backgroundColor=[UIColor blueColor];
    ...
    if(aButton) {
        ...
        CGRect aResizedFrame= self.btnFoldButton.frame;
        aResizedFrame.origin.x=0;//aUpperView.frame.size.width/2-aResizedFrame.size.width/2;
        aResizedFrame.origin.y=aUpperView.frame.size.height-aResizedFrame.size.height/2;
        //self.btnFoldButton=[[UIButton alloc] initWithFrame:aResizedFrame];
        //self.btnFoldButton.backgroundColor=aButton.backgroundColor;
        
        //[aButton removeFromSuperview];
        [self.view addSubview:self.btnFoldButton];
        
        [self.btnFoldButton layoutSubviews];
...
-(void)panDetected:(UIPanGestureRecognizer*)pan
{
    ...
    //currentPlayButtonOrigin.y+= translation.y;
...
- (CGPoint)getNewPointForOffset:(CGFloat)offset withInitial:(CGPoint)ini andFinal:(CGPoint)final
{
    ...
    //x += (status == NowPlayingAnimationStatusInitial) ? ini.x : final.x;
    x+=ini.x;
    
    CGFloat y = (ini.y-final.y)*d;
    //y += (status == NowPlayingAnimationStatusInitial) ? ini.y : final.y;
    y += ini.y;
